...
function Select2AjaxLoader(element,url,failErrorMessage,valueMember,displayMember,showAllOption = false)
{
    let className = [...Array(30)].map(() => Math.random().toString(36)[2]).join('');
    $(element).before('<div class='+className+'></div>');
    let loadingIcon = $("."+className);
    $(loadingIcon).css({
        'display':'none',
        'width': '15px',
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                $.each(response.data, function (i, item) {
                    let valueHolder = valueMember || "Id";
                    let displayHolder = displayMember || "Name";
                    $(element).append(new Option(item[displayHolder],item[valueHolder],false,false));
                });
                $(loadingIcon).css({'display':'none'});
            },
            error: function (e) {
